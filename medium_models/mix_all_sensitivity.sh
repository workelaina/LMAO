#!/bin/bash
# bash mix_all_sensitivity_4.sh --lora_r 8 --lora_alpha 16
            ...
            for LR_lora in "${lr_lora[@]}"; do
                for LR_base in "${lr_base[@]}"; do
                    ...
                    TYPE=prompt GRID_TAG=$GR_TAG TAG=$TAG STEPS=$STEP TASK=$TASK SEED=$SEED MODEL=$MODEL K=$K \
                        ...
                        --zero_order_optim --lr_scheduler_type "constant" --lora_optimizer "adam" --base_optimizer "sgd" --efficient_zero_order --apply_lora \
                        $@
                done
            done
        done
