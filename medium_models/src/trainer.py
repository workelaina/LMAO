...
class Trainer(LinearHeadTrainer):
    ...
    def create_lora_and_base_optimizers_and_schedulers(self, num_training_steps: int):
        """
        Creates two separate optimizers and schedulers: one for LoRA parameters and one for base model parameters.
        This function is similar in spirit to `create_optimizer_and_scheduler`, but sets up separate groups.

        Assumes:
        - self.model is defined
        - self.args contains necessary training arguments such as learning_rate, weight_decay, etc.
        """

        # Separate parameters into LoRA and base parameters
        lora_params = []
        base_params = []
        no_decay = ["bias", "LayerNorm.weight"]
        ...
        lora_optimizer_grouped_parameters = [
            {
        ...
        # Grouping base parameters
        base_optimizer_grouped_parameters = [
            {
        ...
        if self.args.lora_optimizer == 'adam':
        ...
        elif self.args.lora_optimizer == 'sgd':
            self.lora_optimizer = SGD(
            lora_optimizer_grouped_parameters,
            lr=self.args.lora_learning_rate
            )
        else:
        ...
        if self.args.base_optimizer == 'adam':
        ...
        elif self.args.base_optimizer == 'sgd':
            self.base_optimizer = SGD(
                base_optimizer_grouped_parameters,
                lr=self.args.base_learning_rate
            )
        else:
        ...
        # Create LoRA scheduler
        self.lora_scheduler = get_scheduler(
            self.args.lr_scheduler_type,
    ...
    def train(self, model_path=None, dev_objective=None):
        ...
        if self.args.lora_optimizer is not None and self.args.base_optimizer is not None:
            lora_params = []
            base_params = []
            for name, param in self.model.named_parameters():
                if 'lora' in name:
            ...
            self.create_lora_and_base_optimizers_and_schedulers(num_training_steps=t_total)
            lora_optimizer = self.lora_optimizer
            lora_scheduler = self.lora_scheduler
        ...
        if self.args.local_rank != -1:
            # print("current local_rank is: ---------:", self.args.local_rank)
        ...
        for epoch in range(epochs_trained, int(num_train_epochs)):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # ------------------- Base (ZO fine-tuning) + LORA (backpropagation) -------------------
                if self.args.lora_optimizer is not None and self.args.base_optimizer is not None:
                    # -----------------------------
                    # LoRA update (backprop)
                    ...
                    # Set model to train mode if needed
                    model.train()
                    ...
                    if self.args.fp16 and _use_native_amp:
                        ...
                        self.scaler.unscale_(lora_optimizer)
                        torch.nn.utils.clip_grad_norm_(lora_params, self.args.max_grad_norm)
                        self.scaler.step(lora_optimizer)
                        self.scaler.update()
                    else:
                        loss.backward()
                        torch.nn.utils.clip_grad_norm_(lora_params, self.args.max_grad_norm)
                    ...
                    # -----------------------------
                    # Zero-order optimization on base params
                    # Freeze LoRA params now
                    for p in lora_params:
                        p.requires_grad = False
                    for p in base_params:
                        p.requires_grad = True
                    ...
                    self.named_parameters_to_optim = []
                    for name, param in model.named_parameters():
                        if 'lora' not in name and self.should_optim(name, param):
                            self.named_parameters_to_optim.append((name, param))

                    if self.args.zo_by_layer:
                        assert not self.args.efficient_zero_order, 'preconditioned ZO not implemented for efficient ZO'
                        assert self.args.zero_order_use_trainer_optim, 'Need trainer optim for preconditioned ZO'
                        ...
                        # Clip & step base optimizer
                        if self.args.zero_order_clip_grad:
                            norm = torch.nn.utils.clip_grad_norm_(base_params, self.args.max_grad_norm)
                        else:
                            norm = 0.0
                            for p in base_params:
                        ...
                        if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                            ...
                            logs["grad_norm"] = norm.item() if isinstance(norm, torch.Tensor) else norm
                            logs["learning_rate"] = base_scheduler.get_last_lr()[0]
                            logs["num_zs"] = num_zs
                        ...
                        # Standard zero-order optimization
                        num_zs = self.get_num_samples()
                        if num_zs > 1:
                            assert self.args.zero_order_use_trainer_optim, 'need trainer optim for multiple zs'
                        ...
                        for _ in range(num_zs):
                            ...
                            if self.args.gradient_accumulation_steps > 1:
                                assert self.args.zero_order_use_trainer_optim, 'no grad accumulation for non-trainer ZO'
                            ...
                            if self.args.zero_order_use_trainer_optim:
                                ...
                                # non-trainer path
                                assert self.args.gradient_accumulation_steps == 1, 'no grad accumulation for non-trainer ZO'
                                if self.args.efficient_zero_order:
                                    torch.manual_seed(random_seed)
                                ...
                                for name, param in self.named_parameters_to_optim:
                                    if self.args.efficient_zero_order:
                                        ...
                                        z = random_vector[name]
                                    param.data = param.data - self.args.base_learning_rate * (projected_grad * z + self.args.weight_decay * param.data)
                        ...
                        # Apply base optimizer updates if needed
                        if self.args.zero_order_use_trainer_optim:
                            ...
                            # Already updated params directly without optimizer step
                            if (self.args.logging_steps > 0 and self.state.global_step % self.args.logging_steps == 0) or (
                                self.state.global_step == 1 and self.args.logging_first_step
                            ):
                                logs = {}
                                logs["loss"] = loss1.item()
                                logs["learning_rate"] = self.args.base_learning_rate
                                logs["global_step"] = self.state.global_step
                                logs["zo_forward_step"] = self.state.zo_forward_step
                                logs["max_steps"] = self.args.max_steps
                                logs["max_zo_forward_steps"] = self.args.max_zo_forward_steps
                    ...
                    # Re-enable LoRA params for next iteration
                    # for p in lora_params:
                    #     p.requires_grad = True
                    # for p in base_params:
                    #     p.requires_grad = True
