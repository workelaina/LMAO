...
class DynamicTrainingArguments(TrainingArguments):
    ...
    lora_optimizer: str = field(
        default=None,
        metadata={'help': 'choose sgd or adam for optimizing lora parameters.'}
    )
    base_optimizer: str = field(
        default=None,
        metadata={'help': 'choose sgd or adam for optimizing model parameters.'}
    )
    lora_learning_rate: float = field(
        default=5e-5,
        metadata={'help': 'The initial learning rate for lora_optimizer.'}
    )
    base_learning_rate: float = field(
        default=5e-5, 
        metadata={"help": "The initial learning rate for base_optimizer."}
    ) 
...
def main():
    ...
    if model_args.apply_lora:
        if 'roberta' in model_args.model_name_or_path:
            # print("roberta in model_name_or_path\n ------------------\n------------------------")
