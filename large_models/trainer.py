    ...
    def _inner_training_loop(
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                #-------------------------------------------------------------------
                # 1) Compute the per-step loss / forward pass, depending on trainer
                #-------------------------------------------------------------------
                if args.trainer == "mix":
                    # -------------------
                    # LoRA update portion
                    ...
                    # Standard forward/backward for LoRA
                    if (
                    ...
                    # print("tr_loss_step.item()", tr_loss_step.item())
    ...
    def zo_update(self, model):
        ...
        for name, param in self.named_parameters_to_optim:
            ...
            if "bias" not in name and "layer_norm" not in name and "layernorm" not in name:
                if args.trainer == "mix":
                    param.data = param.data - self.args.base_learning_rate * (self.projected_grad * z + args.weight_decay * param.data)
                else:
                    param.data = param.data - self._get_learning_rate() * (self.projected_grad * z + args.weight_decay * param.data)
            else:
                if args.trainer == "mix":
                    param.data = param.data - self.args.base_learning_rate * (self.projected_grad * z)
                else:
